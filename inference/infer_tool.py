...
from sovitsmodels import SynthesizerTrn
import sys
...
class Svc(object):
    def __init__(self, net_g_path, config_path,
        ...
        if not self.only_diffusion:
            ...
            #whisper model:
            self.whisper_model = utils.get_speech_encoder("whisper-ppg", device=self.dev)

            #prosody model
            #self.prosody_model = utils.get_prosody_encoder()
    ...
    def get_unit_f0(self, wav, tran, cluster_infer_ratio, speaker, f0_filter ,f0_predictor,cr_threshold=0.05, sr=44100):
        ...
        c = utils.ReTrans(c.transpose(1,2), f0.shape[1], 'contentvec', 44100, 512)
        c = c.transpose(1,2).squeeze(0)

        w = self.whisper_model.encoder(wav16k)
        #w = utils.repeat_expand_2d(w.squeeze(0), f0.shape[1],self.unit_interpolate_mode)
        w = utils.ReTrans(w.transpose(1,2), f0.shape[1], 'whisper', 44100, 512)
        w = w.transpose(1,2).squeeze(0)

        # p = self.prosody_model.get_quantized_codes(wav16k)
        # p = utils.repeat_expand_2d(p.transpose(1,2).squeeze(0), f0.shape[1], mode=self.unit_interpolate_mode)
    ...
    def infer(self, speaker, tran, raw_path,
        ...
        if spk_mix:
            c, w, f0, uv = self.get_unit_f0(wav, tran, 0, None, f0_filter,f0_predictor,cr_threshold=cr_threshold, sr=sr)
            n_frames = f0.size(1)
            ...
            c, w, f0, uv = self.get_unit_f0(wav, tran, cluster_infer_ratio, speaker, f0_filter,f0_predictor,cr_threshold=cr_threshold)
            n_frames = f0.size(1)
        c = c.to(self.dtype)
        w = w.to(self.dtype)
        f0 = f0.to(self.dtype)
        ...
        with torch.no_grad():
            ...
            if not self.only_diffusion:
                ...
                audio,f0 = self.net_g_ms.infer(c, w, f0=f0, g=sid, uv=uv,predict_f0=auto_predict_f0, noice_scale=noice_scale,vol=vol)
