...
def run(rank, n_gpus, hps):
    ...
    cs_mi_net = CLUB(192, 2, 512).cuda(rank)
    ps_mi_net = CLUB(192, 1, 512).cuda(rank)
    cp_mi_net = CLUB(2, 1, 512).cuda(rank)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        c, w, f0, spec, y, spk, lengths, uv, volume, p = items
        ...
        #train mi nets
        first_forward(scaler, net_g, cs_mi_net, ps_mi_net, cp_mi_net,  optimizer_cs_mi_net, optimizer_ps_mi_net, optimizer_cp_mi_net, c, w, f0, uv, p, spec, g, lengths ,volume)

        # train gen net
        loss_disc, loss_gen, loss_fm, loss_mel, loss_kl, loss_lf0, mi_loss, loss_gen_all, loss_disc_all, grad_norm_d, grad_norm_g, y_mel, y_hat_mel, lf0, pred_lf0, norm_lf0 = second_forward(scaler, half_type, hps, net_g, net_d, optim_g, optim_d, cs_mi_net, ps_mi_net, cp_mi_net,  c, w, f0, uv, p, spec, g, y, lengths ,volume, mel)
...
def first_forward(scaler, net_g, cs_mi_net, ps_mi_net, cp_mi_net,  optimizer_cs_mi_net, optimizer_ps_mi_net, optimizer_cp_mi_net, c, w, f0, uv, p, spec, g, lengths ,volume):
            
    with torch.no_grad():
         # First forward pass for MI loss calc (without gradients)
        y_hat, ids_slice, z_mask, \
            (z, z_p, m_p, logs_p, m_q, logs_q), pred_lf0, norm_lf0, lf0, z_ptemp, fused, gg, pros, pp = net_g(c, w, f0, uv, p, spec, g=g, c_lengths=lengths,
    ...
    lld_cs_loss = -cs_mi_net.loglikeli(fused, pp)
    ...
    lld_cp_loss = -cp_mi_net.loglikeli(pp, lf0)
...
def second_forward(scaler, half_type, hps, net_g, net_d, optim_g, optim_d, cs_mi_net, ps_mi_net, cp_mi_net,  c, w, f0, uv, p, spec, g, y, lengths ,volume, mel):
    ...
    with autocast(enabled=hps.train.fp16_run, dtype=half_type):
        ...
        with autocast(enabled=False, dtype=half_type):
            ...
            # MI estimation 
            mi_cs_loss = cs_mi_net.mi_est(fused, pp) * 0.01
            mi_ps_loss = ps_mi_net.mi_est(fused, lf0) * 0.01
            mi_cp_loss = cp_mi_net.mi_est(pp, lf0) * 0.01
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, items in enumerate(eval_loader):
            c, w, f0, spec, y, spk, _, uv,volume, p = items
            g = spk[:1].cuda(0)
            ...
            y_hat,_ = generator.module.infer(c, w, f0, uv, g=g,vol = volume)
