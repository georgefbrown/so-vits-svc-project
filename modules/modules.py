...
class AdaINLayer(nn.Module):
    def __init__(self, c_h: int):
        super(AdaINLayer, self).__init__()
        self.c_h = c_h
        self.norm_layer = nn.InstanceNorm1d(c_h, affine=False)
        self.style_layer = nn.Linear(c_h, c_h * 2)

    def forward(self, c: Tensor, g: Tensor) -> Tensor:
        # Normalize c
        c_norm = self.norm_layer(c)

        # Compute Lσ(g) and Lμ(g)
        style = self.style_layer(g.transpose(1, 2))
        L_sigma, L_mu = style.chunk(2, dim=2)
        
        # Apply AdaIN
        out = L_sigma.transpose(1, 2) * c_norm + L_mu.transpose(1, 2)
...
class WN(torch.nn.Module):
  def __init__(self, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0, p_dropout=0):
    ...
    self.adain_layers = torch.nn.ModuleList()  # New list for adain layers
    ...
    for i in range(n_layers):
      ...
      # Add Adain Layer
      adain_layer = AdaINLayer(2*hidden_channels)
      self.adain_layers.append(adain_layer)
  ...
  def forward(self, x, x_mask, g=None, **kwargs):
    ...
    for i in range(self.n_layers):
      ...
      #x_in = self.adain_layers[i](x_in, g_l)
