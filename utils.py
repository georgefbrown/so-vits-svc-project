...
def get_prosody_encoder(device=None):
  
   print("Getting prosody encoding")
   from vencoder.VQwave2vec import VQWave2Vec
   vqwave2vec = VQWave2Vec(device=device)
   return vqwave2vec

def get_speech_encoder(speech_encoder,device=None,**kargs):
    print("Speech encoder is", speech_encoder)
...
def log_for_ReTrans(err):
    # Define the directory and file path
    err_log_dir = "ReTranslogs"
    err_log_file = os.path.join(err_log_dir, "align_max_err.log")
...
def ReTrans(source_feats, padded_target_len, model, sample_rate, hop_size):
        """
        ...
        if model == 'contentvec':
           source_hop = contentvec_frameshift * sample_rate

        if model == 'whisper':
           source_hop = whisper_frameshift * whisper_downsample_rate * sample_rate
