...
def process_one(filename, contentvec, whisper, prosody, f0p, device, diff=False, mel_extractor=None):
    # Load audio file
    ...
    prosodyFeat = prosody.get_quantized_codes(wav16k)
...
def process_batch(file_chunk, f0p, diff=False, mel_extractor=None, device="cpu"):
    ...
    for filename in tqdm(file_chunk, position = rank):
        process_one(filename, contentvec, whisper, prosody, f0p, device, diff, mel_extractor)
